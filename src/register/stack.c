...
void set_stack_pointer(uint16_t stack_pointer_address) {
  stack_pointer = stack_pointer_address;
}
...
void push_word(uint16_t word_value) {
  ...
  write(get_stack_pointer(), low_byte);
  write(get_stack_pointer() + 1, high_byte);
}

uint16_t pull_word() {
  set_stack_pointer((get_stack_pointer() + 2) & 0xFFFF);

  uint8_t low_byte = read(get_stack_pointer() - 2);
  uint8_t high_byte = read(get_stack_pointer() - 1);
