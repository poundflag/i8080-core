...
uint8_t alu_add(uint8_t value_one, uint8_t value_two, bool carry_bit) {
  set_register(REG_F, process_flag_register(value_one, value_two + carry_bit,
                                            PLUS_OPERATION));
...
uint8_t alu_and(uint8_t value_one, uint8_t value_two) {
  set_register(REG_F,
               process_flag_register(value_one & value_two, 0, PLUS_OPERATION));
...
bool adi(int machine_cycle) {
  ...
  case 1:
    uint8_t result =
        alu_add(get_register(REG_A), read(get_program_counter()), false);
    set_register(REG_A, result);
    return true;
  }
  return false;
...
bool adc(Register source) {
  uint8_t result = alu_add(get_register(REG_A), get_register(source),
...
bool aci(int machine_cycle) {
  ...
  case 0:
    ...
    break;
  case 1:
    uint8_t result = alu_add(get_register(REG_A), read(get_program_counter()),
                             get_register_bit(REG_F, CARRY));
    set_register(REG_A, result);
    return true;
  }
...
bool sbb(Register source) {
  uint8_t result = alu_sub(get_register(REG_A), get_register(source),
                           get_register_bit(REG_F, CARRY));
  set_register(REG_A, result);
  return true;
}
...
bool sbi(int machine_cycle) {
  ...
  case 0:
    ...
    break;
  case 1:
    uint8_t result = alu_sub(get_register(REG_A), read(get_program_counter()),
                             get_register_bit(REG_F, CARRY));
    set_register(REG_A, result);
    return true;
  }
...
bool daa() {
   ...
   * or the upper nibble will be greater than 9 when adding 6 to the lower
   * nibble or the carry flag is set
   */
  if (((get_register(REG_A) & 0xF0) > 0x90) ||
      (((get_register(REG_A) & 0xF0) >= 0x90) &&
       ((get_register(REG_A) & 0xF) > 9)) ||
      get_register_bit(REG_F, CARRY)) {
    add1 |= 0x60;
    carry = 1;
  }
...
bool ani(int machine_cycle) {
  ...
  case 1:
    set_register(REG_A,
                 alu_and(get_register(REG_A), read(get_program_counter())));
    return true;
  }
  return false;
}
...
bool ori(int machine_cycle) {
  ...
  case 1:
    set_register(REG_A,
                 alu_or(get_register(REG_A), read(get_program_counter())));
    return true;
  }
  return false;
}
...
bool xri(int machine_cycle) {
  ...
  case 1:
    set_register(REG_A,
                 alu_xor(get_register(REG_A), read(get_program_counter())));
    return true;
  }
  return false;
}
